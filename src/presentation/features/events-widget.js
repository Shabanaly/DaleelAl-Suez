...
async function initEvents(containerId) {
...
async function fetchEvents() {
    // Try to fetch through Business Layer
    try {
        if (!window.EventsService) {
            console.warn("EventsService missing, utilizing mock.");
            return getMockEvents();
        }

        const data = await window.EventsService.getUpcomingEvents();
    ...
    // Return mock data if no real events
    return getMockEvents();
}
