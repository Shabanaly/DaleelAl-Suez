...
async function initSuezLive(widgetId) {
...
async function fetchWeather(widget) {
    try {
        if (!window.WeatherService) throw new Error("WeatherService not found");
        
        const weather = await window.WeatherService.getCurrentWeather();
