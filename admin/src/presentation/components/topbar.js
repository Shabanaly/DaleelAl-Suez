const Topbar = {
  render: (pageTitle = "لوحة التحكم") => {
    ...
    const user = (window.AppAuthService && window.AppAuthService.getUserData && window.AppAuthService.getUserData()) ? window.AppAuthService.getUserData() : { email: 'Admin', name: 'Admin' };
    const userEmail = user?.email || "المدير";
    const userName = user?.name || "Admin";

    console.log("🔝 Top Bar - بيانات المستخدم:", user);
            ...
            <header class="topbar">
                ...
                <div style="display:flex; align-items:center; gap: 16px;">
                    ...
                    <div style="display:flex; align-items:center; gap:8px; cursor:pointer; padding: 8px; border-radius: 8px; transition: background 0.2s;" 
                         ...
                         title="آخر نشاط: ${user?.lastActivity ? new Date(user.lastActivity).toLocaleString("ar-EG") : "N/A"}">
                        <img src="https://placehold.co/64x64/2563eb/ffffff?text=${(userName.charAt(0) || 'A').toUpperCase()}" style="width:32px; height:32px; border-radius:50%;">
